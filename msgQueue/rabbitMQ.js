...
class RabbitMqMessageQueue {
  ...
  connectToRabbitMQ = async () => {
    try {
      ...
      const isQueuePresent = await channel.assertQueue('book-store', {
        durable: true,
      });
  ...
  sendToQueue = async (options) => {
    try {
      ...
      channel.sendToQueue('book-store', Buffer.from(JSON.stringify(message)));
  ...
  consumeFromQueue = async () => {
    try {
      const channel = await this.connectToRabbitMQ();
      channel.consume('book-store', (message) => {
        const input = JSON.parse(message.content.toString());
        sendMailNotification(input);
